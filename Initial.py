...
def on_add_button_click(event):
    ...
    elif item in [sort_butt,sort_text]:  
        sort_bg.itemconfig(sort_butt, fill='#153E83')
        sort_text.config( bg="#153E83",fg='#FFFFFF')
...
def filter_students():
    global students  # Ensure we're working with the correct list

    query = search_var.get().strip().lower().replace(",", "").replace("  ", " ")

    # ✅ **Clear the Treeview before inserting new filtered data**
    ...
    for student in students:
        ...
        # ✅ **Ensure "Last, First" and "First Last" name formats are searchable**
        ...
        if any(query in value for value in student_values) or query in original_name or query in reversed_name:
            tree.insert("", "end", values=student)  # ✅ Display matching rows
...
def sort_students():
    global students, name_sort, id_sort, tree

    id_order = id_sort.get()
    name_order = name_sort.get()

    if not id_order and not name_order:
        return  # Do nothing if neither sorting option is selected

    # Sorting order conversion
    id_reverse = id_order == "Descending"
    name_reverse = name_order == "Descending"

    def parse_id(id_str):
        """ Convert ID No. (YYYY-NNNN) into a tuple (YYYY, NNNN) for proper sorting """
        try:
            year, number = map(int, id_str.split("-"))  # Example: "2023-1234" → (2023, 1234)
            return (year, number)
        except ValueError:
            return (float('inf'), float('inf'))  # Push invalid IDs to the bottom

    # ✅ **Sorting Logic Based on Selection**
    if name_order and id_order:
        students.sort(key=lambda x: (parse_id(x[0]), x[1].lower()), reverse=id_reverse)  # ID → Name
    elif id_order:
        students.sort(key=lambda x: parse_id(x[0]), reverse=id_reverse)
    elif name_order:
        students.sort(key=lambda x: x[1].lower(), reverse=name_reverse)
    ...
    print(f"Sorting by - Name: {name_order}, ID: {id_order}")



def sort_click_release(event):
    global sorting,sort_bg,sort_butt,sort_text,id_sort,name_sort
    ...
    if sorting:
        ...
        def on_sort_click(event):
            sort_bg.itemconfig(sort_butt, fill='#154BA6')
            sort_text.config(bg='#154BA6', fg='white') 
            sort_students()
            print("clicked!")
        ...
        sort_by = Label(root,text="Sort By:", font=("Arial", 10, "bold"), bg="light gray",fg="black")
        ...
        name_sort.set("Name") 
        name_sort.bind("<<ComboboxSelected>>",on_select)
        ...
        id_sort.set("ID No.") 
        id_sort.bind("<<ComboboxSelected>>",on_select)


        sort_bg = Canvas(root,bg="lightgray",width=50,height=30,bd=0,highlightthickness=0)
        sorting.create_window(45,112,window=sort_bg)
        sort_butt = create_rounded_rectangle(sort_bg, 0, 0, 50, 30, radius=20, fill="#2363C6") 
        sort_text = Label(root,text="SORT", font=("Arial", 10, "bold"), bg="#2363C6",fg="white")
        sorting.create_window(45,112,window=sort_text)


        sort_bg.tag_bind(sort_butt,"<Button-1>",on_add_button_click)
        sort_bg.tag_bind(sort_butt,"<ButtonRelease-1>",on_sort_click)

        sort_text.bind("<Button-1>",on_add_button_click)
        sort_text.bind("<ButtonRelease-1>",on_sort_click)

        sort_bg.bind("<Enter>",on_sort_hover)
        sort_bg.bind("<Leave>",on_sort_leave)

        sort_text.bind("<Enter>",on_sort_hover)
        sort_text.bind("<Leave>",on_sort_leave)
