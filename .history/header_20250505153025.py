...
def header(root):
    ...
    # Sort button
    sort_canvas = Canvas(top, bg="red",width=30, height=30)
    sort_canvas.grid(row=0, column=3, padx=(0, 10), pady=0, sticky="e")

    sort = create_rounded_rectangle(sort_canvas,0, 0, 30, 30,radius=20, fill="blue")
    icon2 = PhotoImage(file="Images/Sort.png")
    sort_img = sort_canvas.create_image(10, 15, image=icon2, anchor="center")

    sort_canvas.tag_bind(sort, "<Button-1>", sort_clicked)
