...
def header(root):
    ...
    top = Frame(root, bg="white", height=50)
    ...
    search_canvas = Canvas(search_frame, bg="white",width= 400, height=40, bd=0, highlightthickness=0)
    search_canvas.grid(row=0, column=1, pady=5, padx=(50, 120),sticky="nsew")
...
def sort_clicked(event=None):
    sort_canvas.itemconfig(sort, fill="#1A4A99")  # Dark blue for click effect
    
    def on_sort_release(event):
        # We don't reset the color here - let sorting.py handle it
        pass
    
    sort_canvas.bind("<ButtonRelease-1>", on_sort_release)

    from body import body_frame
    from sorting import sort_function
    sort_function(body_frame)
