...
from misc import create_rounded_rectangle,FormManager,bind_button_effects,remove_focus
...
def add_college_form(root, body_frame=None, edit_mode=False, college_id=None, college_name=None, college_code=None):
    ...
    form_frame2.bind("<Button-1>",remove_focus)
    ...
    def submit_form():
        ...
        code = college_code_entry.get().strip().upper()
        ...
        if any(char.isdigit() for char in college_name):
            messagebox.showerror("Error", "College name should not contain numbers")
            return
        
        if any(char.isdigit() for char in code):
            messagebox.showerror("Error", "College code should not contain numbers")
            return
