...
current_sort_field = "id_number"  # Track current sort field
current_sort_direction = "ascending"  # Track current sort direction
...
def sort_function(body_frame):
    ...
    sort_direction = StringVar(value=current_sort_direction)
    sort_field = StringVar(value=current_sort_field)
    ...
    button_canvas.tag_bind(apply, "<Leave>", lambda e: button_canvas.itemconfig(apply_bg, fill="#3A7FF6"))
    ...
    button_canvas.tag_bind(cancel, "<Leave>", lambda e: button_canvas.itemconfig(cancel_bg, fill="#AA4141"))
...
def apply_sort(field, direction):
    ...
    if field == "last_name":
        # Force the field to be last_name for proper SQL sorting
        field = "last_name"
    ...
    # Hide the canvas after applying
    toggle_visibility()
