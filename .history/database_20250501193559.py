...
# Suggested modifications for the get_all_students function in database.py

def get_all_students(page=1, items_per_page=10, search_term=None):
    """
    Get all students with pagination and search capabilities
    
    Args:
        page (int): Page number (1-based)
        items_per_page (int): Number of items per page
        search_term (str): Search term to filter students
        
    Returns:
        tuple: (list of student dictionaries, total count)
    """
    ...
    # Base SQL query for fetching student data
    sql = """
    SELECT s.id, s.id_number, s.first_name, s.last_name, s.gender, s.year_level,
           c.college_code, p.program_code
    FROM students s
    ...
    if search_term and search_term.strip():
        import search
        search_criteria = search.parse_search_query(search_term)
        
        # Handle field-specific search
        where_clauses = []
        
        if "general" in search_criteria:
            general_term = search_criteria["general"]
            if general_term:
                ...
                where_clauses.append("""
                    (s.id_number LIKE ? OR 
                     ...
                     s.last_name LIKE ? OR 
                     c.college_code LIKE ? OR 
                     c.college_name LIKE ? OR
                     p.program_code LIKE ? OR 
                     p.program_name LIKE ? OR
                     s.gender LIKE ? OR
                     CAST(s.year_level AS TEXT) LIKE ?)
                """)
                params.extend([f"%{general_term}%"] * 9)  # Add the parameter for each field
        else:
            # Field-specific searches
            if "id_number" in search_criteria:
                where_clauses.append("s.id_number LIKE ?")
                params.append(f"%{search_criteria['id_number']}%")
                
            if "name" in search_criteria:
                where_clauses.append("(s.first_name LIKE ? OR s.last_name LIKE ?)")
                params.extend([f"%{search_criteria['name']}%"] * 2)
                
            if "college" in search_criteria:
                where_clauses.append("(c.college_code LIKE ? OR c.college_name LIKE ?)")
                params.extend([f"%{search_criteria['college']}%"] * 2)
                
            if "program" in search_criteria:
                where_clauses.append("(p.program_code LIKE ? OR p.program_name LIKE ?)")
                params.extend([f"%{search_criteria['program']}%"] * 2)
                
            if "gender" in search_criteria:
                where_clauses.append("s.gender LIKE ?")
                params.append(f"%{search_criteria['gender']}%")
                
            if "year_level" in search_criteria:
                # Try to handle both text (1st, 2nd) and numeric (1, 2) year values
                year_value = search_criteria["year_level"].lower()
                
                # Handle textual year descriptions
                year_mapping = {"1st": 1, "2nd": 2, "3rd": 3, "4th": 4, "5th": 5}
                if year_value in year_mapping:
                    where_clauses.append("s.year_level = ?")
                    params.append(year_mapping[year_value])
                elif year_value.isdigit():
                    where_clauses.append("s.year_level = ?")
                    params.append(int(year_value))
                else:
                    # Fuzzy matching for year level
                    where_clauses.append("CAST(s.year_level AS TEXT) LIKE ?")
                    params.append(f"%{year_value}%")
        
        # Apply where clauses if any were added
        if where_clauses:
            where_clause = " WHERE " + " OR ".join(where_clauses)
            sql += where_clause
            count_sql += where_clause
    
    # Execute the count query to get total matching records
    # cursor = conn.cursor()
    ...
    # Execute the main query to get student data
