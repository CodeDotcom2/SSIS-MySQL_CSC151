...
def sort_function(body_frame):
    ...
    # Import sidebar resize control functions
    from side import disable_resizing, enable_resizing

    # Disable resizing before we toggle sort panel
    disable_resizing()
    ...
    if sort_canvas and sort_canvas.winfo_exists():
        if is_visible:
            ...
            # Re-enable resizing after closing sort panel with a small delay
            body_frame.after(100, enable_resizing)
            return
        else:
...
def toggle_visibility():
    ...
    # Disable resizing during this operation
    disable_resizing()
    ...
    if sort_canvas:
        if is_visible:
            ...
            # Re-enable resizing after closing sort panel with a small delay
            sort_canvas.after(100, enable_resizing)
        else:
            sort_canvas.place(relx=0.95, rely=0.0, anchor="ne")
            is_visible = True
