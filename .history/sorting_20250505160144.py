from tkinter import *
from tkinter import ttk
from misc import create_rounded_rectangle

# Global variables
sort_canvas = None
sort_frame = None
is_visible = False

def sort_function(body_frame):
    """
    Creates a sorting form on a canvas within the body frame
    """
    global sort_canvas, sort_frame, is_visible
    
    # If the sorting canvas already exists, toggle its visibility
    if sort_canvas and sort_canvas.winfo_exists():
        if is_visible:
            sort_canvas.place_forget()
            is_visible = False
            return
        else:
            # Show the existing canvas
            sort_canvas.place(relx=0.5, rely=0.5, anchor=CENTER)
            is_visible = True
            return
    
    # Create a new canvas for the sorting form
    sort_canvas = Canvas(body_frame, bg="#F0F0F0", width=350, height=480, 
                       highlightthickness=0, bd=0)
    
    # Position the canvas in the center of the body frame
    sort_canvas.place(relx=0.5, rely=0.5, anchor=CENTER)
    is_visible = True
    
    # Create a rounded rectangle background
    create_rounded_rectangle(sort_canvas, 10, 10, 340, 470, radius=20, fill='white', 
                           outline="#E4EBF5", width=2)
    
    # Add a shadow effect
    shadow = create_rounded_rectangle(sort_canvas, 15, 15, 345, 475, radius=20, 
                                   fill='#E4EBF5', outline="")
    sort_canvas.tag_lower(shadow)  # Move shadow behind the white background
    
    # Create a title
    sort_canvas.create_text(175, 50, text="Sort Students By", 
                          font=("Albert Sans", 18, "bold"), fill="#333333")
    
    # Container frame for all sort options
    sort_frame = Frame(sort_canvas, bg="white", bd=0, highlightthickness=0)
    sort_canvas.create_window(175, 250, window=sort_frame)
    
    # Sort options
    sort_options = [
        ("ID Number", "id_number"),
        ("Name", "last_name"),
        ("Gender", "gender"),
        ("Year Level", "year_level"),
        ("College", "college_code"),
        ("Program", "program_code")
    ]
    
    # Default sort direction
    sort_direction = StringVar(value="ascending")
    
    # Default sort field
    sort_field = StringVar(value="id_number")
    
    # Create radio buttons for each sort option with custom styling
    style = ttk.Style()
    style.configure("Sort.TRadiobutton", 
                  background="white", 
                  font=("Albert Sans", 12),
                  padding=8)
    
    # Create radio buttons for each sort option
    for i, (text, value) in enumerate(sort_options):
        rb = ttk.Radiobutton(
            sort_frame, 
            text=text, 
            value=value,
            variable=sort_field,
            style="Sort.TRadiobutton"
        )
        rb.grid(row=i, column=0, sticky=W, pady=8, padx=20)
    
    # Direction frame
    direction_frame = Frame(sort_frame, bg="white")
    direction_frame.grid(row=len(sort_options), column=0, sticky=W, pady=(20, 10), padx=20)
    
    direction_label = Label(direction_frame, text="Direction:", font=("Albert Sans", 14), 
                           bg="white", fg="#333333")
    direction_label.pack(side=LEFT, padx=(0, 10))
    
    # Radio buttons for sort direction
    asc_rb = ttk.Radiobutton(
        direction_frame, 
        text="Ascending", 
        value="ascending",
        variable=sort_direction,
        style="Sort.TRadiobutton"
    )
    asc_rb.pack(side=LEFT, padx=(0, 10))
    
    desc_rb = ttk.Radiobutton(
        direction_frame, 
        text="Descending", 
        value="descending",
        variable=sort_direction,
        style="Sort.TRadiobutton"
    )
    desc_rb.pack(side=LEFT)
    
    # Button frame
    button_frame = Frame(sort_frame, bg="white")
    button_frame.grid(row=len(sort_options)+1, column=0, sticky=E, pady=20, padx=20)
    
    # Apply button with hover effect
    apply_button = Button(
        button_frame,
        text="Apply",
        font=("Albert Sans", 12),
        bg="#3A7FF6",
        fg="white",
        padx=15,
        pady=5,
        relief="flat",
        cursor="hand2",
        activebackground="#2A6FD6",
        activeforeground="white",
        command=lambda: apply_sort(sort_field.get(), sort_direction.get())
    )
    apply_button.pack(side=RIGHT, padx=5)
    
    # Cancel button with hover effect
    cancel_button = Button(
        button_frame,
        text="Cancel",
        font=("Albert Sans", 12),
        bg="#f0f0f0",
        fg="black",
        padx=15,
        pady=5,
        relief="flat",
        cursor="hand2",
        activebackground="#e0e0e0",
        activeforeground="black",
        command=lambda: toggle_visibility()
    )
    cancel_button.pack(side=RIGHT, padx=5)
    
    # Add a close "X" button in the top right corner
    close_button = Button(
        sort_canvas,
        text="✕",
        font=("Albert Sans", 12, "bold"),
        bg="white",
        fg="#666666",
        bd=0,
        relief="flat",
        cursor="hand2",
        activebackground="white",
        activeforeground="#333333",
        command=lambda: toggle_visibility()
    )
    sort_canvas.create_window(325, 25, window=close_button)
    
    # Adding subtle visual effects
    separator = sort_canvas.create_line(50, 80, 300, 80, fill="#E4EBF5", width=2)

def toggle_visibility():
    """
    Toggle the visibility of the sorting canvas
    """
    global sort_canvas, is_visible
    if sort_canvas:
        if is_visible:
            sort_canvas.place_forget()
            is_visible = False
        else:
            sort_canvas.place(relx=0.5, rely=0.5, anchor=CENTER)
            is_visible = True

def apply_sort(field, direction):
    """
    Apply the sorting options and hide the canvas
    """
    print(f"Sorting by {field} in {direction} order")
    
    # TODO: Implement actual sorting logic here
    # This would connect to your database query or modify how data is displayed
    
    # Hide the canvas
    toggle_visibility()
    
    # TODO: Refresh the student table with the sorted data
    # This would call a function in your body.py to refresh the display
    from body import refresh_students
    refresh_students()