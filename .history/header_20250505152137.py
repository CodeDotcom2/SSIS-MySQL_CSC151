...
def header(root):
    ...
    # Sort button
    sort_canvas = Canvas(top, bg="red",width=20, height=30)
    sort_canvas.grid(row=0, column=3, padx=(0, 10), pady=0, sticky="e")

    sort = create_rounded_rectangle(sort_canvas,20,30, 20, 30,radius=20, fill="red", outline="blue")
    icon2 = PhotoImage(file="Images/Sort.png")
    #sort_button = Button(sort_canvas, image=icon2, bg="white", bd=0, cursor="hand2",
    #                     command=sort_clicked)
    #sort_button.grid(row=0, column=3, padx=(0, 80), pady=5,sticky="e")
    search_frame.columnconfigure(1, weight=1)

    def on_sort_hover(event):
        sort_button.config(bg="#E4EBF5")
    def on_sort_leave(event):
        sort_button.config(bg="white")
    #sort_button.bind("<Enter>", on_sort_hover)

    #sort_button.bind("<Leave>", on_sort_leave)
