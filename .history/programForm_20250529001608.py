...
def add_program_form(root, body_frame=None, edit_mode=False, program_id=None, program_name=None, program_code=None, college_id=None):
    ...
    form_frame3.bind("<Button-1>",remove_focus)
    ...
    def submit_form():
        from body import refresh_students
        name = add_name.get()
        code = program_code_entry.get().strip().upper()
        ...
        if not re.fullmatch(r"[A-Za-z\s\-\(\)]+", name):
            messagebox.showerror("Input Error", "Program name must contain only letters, dashes, and parentheses (no numbers allowed).")
            return 
        
        if not re.fullmatch(r"[A-Za-z\s\-\(\)]+", code):
            messagebox.showerror("Input Error", "Program code must contain only letters, dashes, and parentheses (no numbers allowed).")
            return
        ...
        if edit_mode:
            ...
            if success:
                ...
                def reset_dropdown_style():
                    college_dropdown.configure(style="Gray.TCombobox")
                    college_dropdown.update()
                
                college_dropdown.after(50, reset_dropdown_style)  
