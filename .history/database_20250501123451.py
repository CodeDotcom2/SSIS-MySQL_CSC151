...
def get_all_students(page=1, items_per_page=10, search_term=None):
    ...
    if connection is not None:
        try:
            ...
            # Get paginated results - Modified to handle deleted programs better
            query = """
                SELECT 
                    s.id, s.first_name, s.last_name, s.id_number, 
                    s.year_level, s.gender, s.program_id,
                    COALESCE(p.name, 'N/A') as program_name,
                    COALESCE(p.code, 'N/A') as program_code,
                    COALESCE(c.id, (SELECT pc.college_id FROM programs pc WHERE pc.id = s.program_id)) as college_id,
                    COALESCE(c.name, (SELECT pc.name FROM colleges pc WHERE pc.id = (SELECT pcc.college_id FROM programs pcc WHERE pcc.id = s.program_id)), 'N/A') as college_name,
                    COALESCE(c.code, (SELECT pc.code FROM colleges pc WHERE pc.id = (SELECT pcc.college_id FROM programs pcc WHERE pcc.id = s.program_id)), 'N/A') as college_code
                FROM students s
                LEFT JOIN programs p ON s.program_id = p.id
                LEFT JOIN colleges c ON p.college_id = c.id
            """
