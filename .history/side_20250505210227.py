...
current_sidebar_width = 180
resizing_enabled = True  # Flag to control when resize should happen
...
def side_buttons(body_frame):
    global side_bar_canvas, side_frame, button, icon, text_item
    global icon_images
    ...
    button_specs = [
        ("Add Student", "white", "", 160, 35, 35, lambda event: on_add_button_click(event, body_frame=body_frame)),
        ("Delete Student", "#2363C6", "Images/Trash.png", 160, 25, 15, delete_stud),
        ("Edit Student", "#2363C6", "Images/edit.png", 160, 25, 2, lambda event: edit_stud(event, body_frame=body_frame)),
        ("Colleges", "#2363C6", "Images/college.png", 160, 25, 2, lambda event: colleges_func(event, body_frame=body_frame)),
        ("Programs", "#2363C6", "Images/program.png", 160, 25, 2, lambda event: programs_func(event, body_frame=body_frame))
    ]
...
def side(root, body_frame):
    global side_bar_canvas, current_sidebar_width, resizing_enabled, side_frame
    ...
    # Set initial width
    current_sidebar_width = 180
    root.columnconfigure(0, minsize=current_sidebar_width)

    # Variable to store last window width
    root.last_window_width = root.winfo_width()

    def on_root_resize(event):
        global current_sidebar_width, resizing_enabled
        ...
        # Check if we're in a genuine resize or if this is triggered by sort button
        # A small width change (less than 10px) while sorting is active should be ignored
        if not resizing_enabled:
            # Don't adjust sidebar during sorting operations
            return
            
        # This is a genuine window resize
        # Calculate new sidebar width (20% of window width, between 180-250px)
        sidebar_width = max(180, min(250, int(total_width * 0.2)))
        ...
        # Update last known window width
        root.last_window_width = total_width

    root.bind("<Configure>", on_root_resize)

    def make_side_bar_handler(body_frame):
        def side_bar(event):
            side_buttons(body_frame)
            side_frame.bind("<Button-1>", remove_focus)
        return side_bar

    side_bar_canvas.bind("<Configure>", make_side_bar_handler(body_frame))
    
    return side_frame, side_bar_canvas

def disable_resizing():
    """External function to disable sidebar resizing temporarily"""
    global resizing_enabled
    resizing_enabled = False
    
def enable_resizing():
    """External function to re-enable sidebar resizing"""
    global resizing_enabled
