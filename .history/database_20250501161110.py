    ...
    if connection is not None:
        try:
            ...
            count_query = """
                ...
                LEFT JOIN colleges c ON (p.college_id = c.id OR 
                                       (s.program_id IS NULL AND EXISTS (
                                           SELECT 1 FROM programs p2 
                                           WHERE p2.college_id = c.id 
                                           AND p2.id = (SELECT program_id FROM students s2 
                                                       WHERE s2.id = s.id LIMIT 1)
                                       ))
            """
            ...
            # Get paginated results with improved college retrieval
            query = """
                SELECT 
                    s.id, 
                    s.first_name, 
...
def delete_program(program_id):
    ...
    if connection is not None:
        try:
            ...
            # Delete the program (this will set student.program_id to NULL due to ON DELETE SET NULL)
            cursor.execute("DELETE FROM programs WHERE id = %s", (program_id,))
