...
def add_program_form(root, body_frame=None, edit_mode=False, program_id=None, program_name=None, program_code=None, college_id=None):
    ...
    def force_gray_combobox(combobox):
        combobox.configure(style="Gray.TCombobox")
        combobox.update_idletasks()
        # Temporary workaround for Tkinter styling issues
        combobox.configure(font=('Arial', 11))
        combobox.update_idletasks()
        combobox.configure(style="Gray.TCombobox")
    def submit_form():
        from body import refresh_students
