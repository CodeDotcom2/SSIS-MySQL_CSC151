...
def sort_function(body_frame):
    ...
    direction_label = Label(sort_frame, text="Direction:", font=("Albert Sans", 14), 
                         bg="white", fg="#333333")
    direction_label.grid(row=len(sort_options), column=0, sticky=W, pady=(5, 0), padx=20)
    
    direction_frame = Frame(sort_frame, bg="white")
    direction_frame.grid(row=len(sort_options)+1, column=0, sticky=W, pady=0, padx=20)
    ...
    button_canvas = Canvas(sort_frame, bg="white", width=230, height=50, highlightthickness=0)
    button_canvas.grid(row=len(sort_options)+2, column=0, pady=(15, 5))

    # Create rounded rectangle buttons
    apply_bg = create_rounded_rectangle(button_canvas, 10, 5, 110, 45, radius=20, fill="#3A7FF6", outline="#3A7FF6")
    cancel_bg = create_rounded_rectangle(button_canvas, 120, 5, 220, 45, radius=20, fill="#f0f0f0", outline="#f0f0f0")

    # Add button text
    button_canvas.create_text(60, 25, text="Apply", font=("Albert Sans", 12), fill="white")
    button_canvas.create_text(170, 25, text="Cancel", font=("Albert Sans", 12), fill="black")

    # Add button click functionality
    button_canvas.tag_bind(apply_bg, "<Button-1>", lambda e: apply_sort(sort_field.get(), sort_direction.get()))
    button_canvas.tag_bind(button_canvas.create_text(60, 25), "<Button-1>", lambda e: apply_sort(sort_field.get(), sort_direction.get()))
    
    button_canvas.tag_bind(cancel_bg, "<Button-1>", lambda e: toggle_visibility())
    button_canvas.tag_bind(button_canvas.create_text(170, 25), "<Button-1>", lambda e: toggle_visibility())

    # Add hover effects
    button_canvas.tag_bind(apply_bg, "<Enter>", lambda e: button_canvas.itemconfig(apply_bg, fill="#2A6FD6"))
    button_canvas.tag_bind(apply_bg, "<Leave>", lambda e: button_canvas.itemconfig(apply_bg, fill="#3A7FF6"))
    
    button_canvas.tag_bind(cancel_bg, "<Enter>", lambda e: button_canvas.itemconfig(cancel_bg, fill="#e0e0e0"))
    button_canvas.tag_bind(cancel_bg, "<Leave>", lambda e: button_canvas.itemconfig(cancel_bg, fill="#f0f0f0"))
