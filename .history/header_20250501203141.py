...
def header(root):
    ...
    search_canvas = Canvas(search_frame, bg="blue", height=30, bd=0, highlightthickness=0)
    search_canvas.grid(row=0, column=1, sticky="nsew", pady=5,padx=(0, 10))
    ...
    def resize_search_bar(event):
        ...
        create_rounded_rectangle(search_canvas, 2, 2, width-2, 30, radius=20,
                                fill='red', outline="#E4EBF5", tag="rounded_bg")
        search_canvas.coords(search_bar_window, 10, 16)  # Keep position
        search_canvas.itemconfig(search_bar_window, width=width-20)  # Resize Entry widget
