        ...
        try:
            ...
            if search_term:
                ...
                if 'id_number' in filters:
                    id_num = filters['id_number'].replace('-', '')  # Search with or without hyphen
                    where_clauses.append("(REPLACE(s.id_number, '-', '') LIKE %s)")
                ...
                if 'gender' in filters:
                    ...
                    if gender in ['m', 'male']:
                        where_clauses.append("s.gender = 'Male'")
                    elif gender in ['f', 'female']:
                        where_clauses.append("s.gender = 'Female'")
                    else:
                        where_clauses.append("s.gender LIKE %s")
                        params.append(f"%{filters['gender']}%")
                
                if 'year_level' in filters:
                    if isinstance(filters['year_level'], int):
                        where_clauses.append("s.year_level = %s")
                        params.append(filters['year_level'])
                    else:
                        where_clauses.append("s.year_level LIKE %s")
                        params.append(f"%{filters['year_level']}%")
                
                if 'college' in filters:
                    where_clauses.append("(c.name LIKE %s OR c.code LIKE %s)")
                    params.extend([f"%{filters['college']}%", f"%{filters['college']}%"])
                
                if 'program' in filters:
                    where_clauses.append("(p.name LIKE %s OR p.code LIKE %s)")
                    params.extend([f"%{filters['program']}%", f"%{filters['program']}%"])
                
                # Handle general search terms
                if 'general' in filters:
                    general_clauses = []
                    for term in filters['general']:
                        general_clauses.extend([
                            "s.first_name LIKE %s",
                            "s.last_name LIKE %s",
                            "REPLACE(s.id_number, '-', '') LIKE %s",
                            "s.gender LIKE %s",
                            "c.name LIKE %s",
                            "c.code LIKE %s",
                            "p.name LIKE %s",
                    ...
                    where_clauses.append(f"({' OR '.join(general_clauses)})")
