...
def side(root,body_frame):
    global side_bar_canvas, current_sidebar_width
    ...
    # Initialize current_sidebar_width
    current_sidebar_width = 180


    def on_root_resize(event):
        global current_sidebar_width
        
        # Check if event is coming from the root window
        if event.widget == root:
            # Get the current sort panel state
            from sorting import is_visible
            
            total_width = root.winfo_width()
            
            # Only update sidebar width if sort panel is not active
            # or if we're handling the initial resize
            if not is_visible or not hasattr(root, 'initial_resize_done'):
                sidebar_width = max(180, min(250, int(total_width * 0.2)))
                
                # Update the current sidebar width
                current_sidebar_width = sidebar_width
                
                # Configure the column and frame
                root.columnconfigure(0, minsize=sidebar_width)
                side_frame.config(width=sidebar_width)
            else:
                # Use the stored sidebar width
                root.columnconfigure(0, minsize=current_sidebar_width)
                side_frame.config(width=current_sidebar_width)
                
            # Update canvas width
            canvas_width = side_frame.winfo_width()
            side_bar_canvas.config(width=canvas_width)
            
            # Mark that initial resize is done
            if not hasattr(root, 'initial_resize_done'):
                root.initial_resize_done = True
