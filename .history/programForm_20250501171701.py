...
from database import get_program_with_college
...
def add_program_form(root, body_frame=None, edit_mode=False, program_id=None, program_name=None, program_code=None, college_id=None):
    ...
    college_options = []
    college_ids = {}
    selected_college_name = ""  # To store the matching college name

    for college in colleges:
        display_name = f"{college['code']} - {college['name']}"
        college_options.append(display_name)
        college_ids[display_name] = college['id']
        
        # Find the matching college for the program being edited
        if edit_mode and college_id and college['id'] == college_id:
            selected_college_name = display_name

    college_var = StringVar()
    college_dropdown = ttk.Combobox(root, textvariable=college_var, 
    ...
    if edit_mode and selected_college_name:
...
def view_programs_form(root, body_frame=None):
    ...
    def edit_selected():
        ...
        # Get program ID from the selected item
        item_values = program_tree.item(selected_item[0], "values")
        program_id = item_values[0]
        
        # Get full program details including college
        program = get_program_with_college(program_id)
        if not program:
            messagebox.showerror("Error", "Could not load program details")
            return
            
        form_frame.destroy()
        edit_form = add_program_form(root, body_frame, 
                                edit_mode=True, 
