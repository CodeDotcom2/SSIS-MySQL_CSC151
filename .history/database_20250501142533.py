...
def create_default_na_programs():
    """Ensures every college has an 'N/A' program for deleted program reassignments."""
    connection = create_connection()
    if connection:
        try:
            cursor = connection.cursor(dictionary=True)
            
            # Get all colleges (and fetch all results)
            cursor.execute("SELECT id FROM colleges")
            colleges = cursor.fetchall()  # ← This consumes the query results
            
            for college in colleges:
                college_id = college['id']
                
                # Check if an "N/A" program exists (fetch the result immediately)
                cursor.execute("""
                    SELECT id FROM programs 
                    WHERE college_id = %s AND code = 'N/A'
                """, (college_id,))
                existing_program = cursor.fetchone()  # ← Consumes the result
                
                if not existing_program:  # If no "N/A" program exists, create one
                    cursor.execute("""
                        INSERT INTO programs (name, code, college_id)
                        VALUES ('N/A (Deleted Program)', 'N/A', %s)
                    """, (college_id,))
            
            connection.commit()
            print("✅ Default 'N/A' programs created for all colleges.")
            return True
        except Error as e:
    ...
    if connection is not None:
        try:
            ...
            cursor.execute("""
                ...
                WHERE college_id = %s AND code = 'N/A'
            """, (college_id,))
            na_program = cursor.fetchone()
            
            if not na_program:
                return False, "Default N/A program not found for this college"
            
            na_program_id = na_program['id']
            
            cursor.execute("""
                UPDATE students 
                SET program_id = %s 
                WHERE program_id = %s
            """, (na_program_id, program_id))
            
            cursor.execute("DELETE FROM programs WHERE id = %s", (program_id,))
            
            connection.commit()
            return True, "Program deleted successfully (students reassigned to N/A)"
