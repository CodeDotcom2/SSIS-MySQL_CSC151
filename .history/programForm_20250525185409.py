...
def add_program_form(root, body_frame=None, edit_mode=False, program_id=None, program_name=None, program_code=None, college_id=None):
    ...
    college_dropdown = ttk.Combobox(root, textvariable=college_var, values=college_options, 
                                style="Gray.TCombobox", width=30, font=('Arial', 11, 'normal'))
    ...
    def submit_form():
        ...
        if edit_mode:
            ...
            if success:
                ...
                # Reset all fields consistently with proper styling
                # Program Name
                add_name.delete(0, END)
                add_name.insert(0, "Ex: Bachelor of Science in Information Technology")
                add_name.config(
                    fg="gray",
                    font=('Albert Sans', 12, 'normal'),
                    justify="center"
                )
                ...
                program_code_entry.delete(0, END)
                program_code_entry.insert(0, "Ex: BSIT")
                program_code_entry.config(
                    fg="gray",
                    font=('Albert Sans', 12, 'normal'),
                    justify="center"
                )
                
                # College Dropdown - comprehensive reset
                college_dropdown.set("Select College")
                college_var.set("Select College")
                college_dropdown.configure(style="Gray.TCombobox")
                
                # Re-bind focus events for Entry widgets
                add_name.bind("<FocusIn>", lambda event: add_name.get() == "Ex: Bachelor of Science in Information Technology" and 
                ...
                program_code_entry.bind("<FocusIn>", lambda event: program_code_entry.get() == "Ex: BSIT" and 
                                    (program_code_entry.delete(0, END), program_code_entry.config(fg="black", justify="left")))
                program_code_entry.bind("<FocusOut>", lambda event: program_code_entry.get() == "" and 
                                    (program_code_entry.insert(0, "Ex: BSIT"), program_code_entry.config(fg="gray", justify="center")))
                
                # Force UI update and remove focus
