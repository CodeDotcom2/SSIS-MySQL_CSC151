    ...
    def on_click(event):
        ...
        if item:
            if item in student_table.selection():
                ...
                student_table.item(item, tags=())  # Clear any tags
                global selected_student_id
                selected_student_id = None
            else:
                # Select
                student_table.selection_set(item)
                student_table.item(item, tags=('selected',))
                on_student_select(event)
        # Force update of hover state
        on_hover(event)
