...
def filter_students():
    # Import the set_search_text function directly where it's needed
    try:
        from body import set_search_text
        set_search_text(search_var.get())
    except ImportError as e:
        print(f"Error importing set_search_text: {e}")

def get_search_term():
    return search_var.get()

def setup_search_connection():
    # This function isn't needed anymore as we're directly calling
    # set_search_text in filter_students. Keeping it for backward compatibility.
    pass