...
def header(root):
    ...
    global icon1,icon2, text_image, cancel_butt, search_field_var, search_field_combo
    ...
    search_field_combo.grid(row=0, column=3, padx=0, pady=5)
    ...
    search_canvas = Canvas(search_frame, bg="red", height=30, bd=0, highlightthickness=0)
    search_canvas.grid(row=0, column=1, sticky="nsew", pady=5, padx=0)
    ...
    search_canvas.search_bar_window = search_canvas.create_window(10, 16, anchor="w", window=search_bar, width=450, height=25)
    ...
    # Bind canvas resize to adjust shape and entry size
    def resize_search_bar(event):
        ...
        search_canvas.itemconfig(search_canvas.search_bar_window, width=width + 100)
    ...
    sort_button = Button(top, image=icon2, bg="white", bd=0, cursor="hand2",
                         command=lambda: print("Sort button clicked"))
    sort_button.grid(row=0, column=3, padx=(0, 80), pady=5)
