...
def save_student(first_name, last_name, id_number, year_level, gender, program_id):
    ...
    if connection is not None:
        try:
            ...
            cursor.execute("SELECT name FROM students WHERE last_name = %s",(last_name,))
            if cursor.fetchone():
        ...
        try:
            ...
            query = """
                SELECT 
                    ...
                    CAST(SUBSTRING_INDEX(s.id_number, '-', 1) AS UNSIGNED) as id_part1,
                    CAST(SUBSTRING_INDEX(s.id_number, '-', -1) AS UNSIGNED) as id_part2
                FROM students s
                LEFT JOIN programs p ON s.program_id = p.id
                LEFT JOIN colleges c ON p.college_id = c.id
            """
            ...
            if search_term:
                ...
                if search_field == "all":
                    ...
                    params = [search_pattern, search_pattern, search_pattern, 
                             search_term,  # Exact match for gender
                    ...
                    # Changed to exact match (case-insensitive)
                    where_clause = " WHERE LOWER(s.gender) = LOWER(%s)"
                    params = [search_term]
            ...
            if sort_field == "id_number":
                if sort_direction == "descending":
                    order_by = "id_part1 DESC, id_part2 DESC"
                else:
                    order_by = "id_part1 ASC, id_part2 ASC"
                ...
                order_by = "id_part1 ASC, id_part2 ASC"  # default
            
            # Build the final query with sorting and pagination
            final_query = query + f" ORDER BY {order_by} LIMIT %s OFFSET %s"
            ...
            for row in cursor.fetchall():
                student = {k: v for k, v in row.items() if k not in ['id_part1', 'id_part2']}
...
def get_all_programs(include_deleted=False):
    ...
    if connection is not None:
        try:
            cursor = connection.cursor(dictionary=True)
            query = """
                SELECT p.id, p.name, p.code, p.college_id, COALESCE(c.name, 'N/A') as college_name, COALESCE(c.code, 'N/A') as college_code,
                (SELECT COUNT(*) FROM students s WHERE s.program_id = p.id) as student_count
                FROM programs p
                LEFT JOIN colleges c ON p.college_id = c.id
                WHERE p.name != 'N/A (Deleted)' OR %s
                ORDER BY p.name
            """
