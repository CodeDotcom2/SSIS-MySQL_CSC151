...
# In database.py, enhance the program operations:
...
def get_program_by_id(program_id):
    ...
    if connection is not None:
        try:
            cursor = connection.cursor(dictionary=True)
            cursor.execute("""
                SELECT p.id, p.name, p.code, p.college_id, 
            ...
            print(f"Error retrieving program: {e}")
        finally:
            connection.close()
    return program

def is_program_referenced(program_id):
    """Check if any students reference this program"""
    connection = create_connection()
    if connection is not None:
        try:
            cursor = connection.cursor()
            ...
            print(f"Error checking program references: {e}")
            return True  # Assume referenced to prevent accidental deletion
        finally:
            connection.close()
    return True
