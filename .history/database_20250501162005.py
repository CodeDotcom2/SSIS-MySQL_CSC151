...
def get_all_students(page=1, items_per_page=10, search_term=None):
    ...
    if connection is not None:
        try:
            ...
            # Base query with improved handling of deleted programs
            query = """
                SELECT 
                ...
                LEFT JOIN colleges c ON (
                    p.college_id = c.id OR 
                    (s.program_id IS NULL AND c.id = (
                        SELECT p2.college_id 
                        FROM programs p2 
                        JOIN students s2 ON p2.id = s2.program_id 
                        WHERE s2.id = s.id 
                        LIMIT 1
                    ))
                )
            ...
            # [Rest of your existing pagination and search logic]
            # ...
...
def delete_college(college_id):
    ...
    if connection is not None:
        try:
            ...
            if student_count > 0:
                return False, "Cannot delete college that has enrolled students"
