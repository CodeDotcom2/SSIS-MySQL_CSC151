...
from misc import create_rounded_rectangle,FormManager,on_select,remove_focus
...
def create_form(root, body_frame=None, mode="add", student_data=None, refresh_callback=None):
    ...
    style = ttk.Style()
    style.configure("Gray.TCombobox",foreground="gray", relief="flat") 
    ...
    form_frame.bind("<Button-1>",remove_focus)
    ...
    gender_dropdown = ttk.Combobox(root, style="Gray.TCombobox", values=["Male", "Female", "Others"], 
    ...
    year_dropdown = ttk.Combobox(root, style="Gray.TCombobox", values=["1st", "2nd", "3rd", "4th", "5+"], 
                               state="readonly", width=14, font=('Arial', 11, 'normal'))
    ...
    # College
    college_dropdown = ttk.Combobox(root, style="Gray.TCombobox", values=college_names, 
    ...
    program_dropdown = ttk.Combobox(root, style="Gray.TCombobox", state="readonly", 
    ...
    def submit_form():
        ...
        if not all(char.isalpha() or char in [' ', '-'] for char in last):
            messagebox.showerror("Error","Last Name must contain only letters, spaces, or hyphens.")
            return
        ...
        if not all(char.isalpha() or char in [' ', '-'] for char in first):
            messagebox.showerror("Error","Last Name must contain only letters, spaces, or hyphens.")
            return
        ...
        if not re.match(r'^\d{4}-\d{4}$', student_id_no):
            messagebox.showerror("Error", "ID number must be in format XXXX-XXXX (e.g. 1234-5678)")
            return
        ...
        if selected_program == "Select Program" or selected_program == "Select College First" or selected_program == "No Programs Available":
            ...
            return
        if program_dropdown.get() == "No programs available":
            messagebox.showerror("Error","No programs available, Please select another college.")
    ...
    if mode == "edit" and student_data:
        ...
        gender_dropdown.configure(style="Black.TCombobox")
        year_dropdown.configure(style="Black.TCombobox")
        college_dropdown.configure(style="Black.TCombobox")
        program_dropdown.configure(style="Black.TCombobox")
        ...
        for display_text, college_id in college_dict.items():
            if college_id == student_data['college_id']:
                ...
                college_found = True
                text_elements['college_dropdown'].event_generate("<<ComboboxSelected>>")
                break
