...
def view_colleges_form(root, body_frame=None):
    ...
    global back_icon, sort_order, sort_button_text, original_colors
    ...
    def college_filter(*args):
        ...
        for college in filtered_colleges:
            row_id = college_table.insert("", "end", values=(
                college['code'],
                college['name'],
                0,  # Placeholder for program count
    ...
    college_search_bar.bind("<FocusIn>", clear_colleges_entry)
    college_search_bar.bind("<FocusOut>", college_restore)
    icon2.bind("<Button-1>", clear_colleges_entry)
    icon3.bind("<Button-1>", clear_colleges_entry)
    colleges_var.trace_add("write", college_search)
    
    # Total students label
    total_students_var = StringVar()
    total_students_var.set("Total Students: 0")
    total_students_label = Label(root, textvariable=total_students_var, font=("Arial", 11), 
                               bg="lightgray", fg="#2363C6")
    form_frame.create_window(585, 420, window=total_students_label)
