...
def body(root):
    ...
    body_frame.after(100, lambda: on_resize(None))
    return body_frame

def calculate_rows_per_page():
    if not hasattr(table_frame, 'winfo_height') or table_frame.winfo_height() <= 1:
        return 10

    table_height = table_frame.winfo_height()
    
    style = ttk.Style()
    style.lookup("StudentTable.Treeview.Heading", "font")
    header_height = 40
    
    row_height = 40
    
    available_height = table_height - header_height - 20 
    
    rows = max(1, available_height // row_height)
    return min(max(rows, 1), 50)

def on_resize(event):
    global items_per_page
    if event.widget == event.widget.winfo_toplevel():
        body_frame.after(100, delayed_resize_check)

def delayed_resize_check():
    global items_per_page
...
def load_students(search_term=None):
    ...
    for student in students:
        ...
        student_table.insert('', 'end', values=(
            ...
            student['gender'],
            year_level,
            college_display,  # Shows original college even if program is deleted
            program_display   # Shows "N/A" if program is deleted
        ))
