...
def view_colleges_form(root, body_frame=None):
    ...
    def college_search(*args):
        if colleges_var.get():
            cancel_butts.place(x=260, y=4)
        else:
            cancel_butts.place_forget()
    # Header
    title_text = Label(root, text="Colleges", font=("Arial", 25, "bold"), bg="lightgray", fg="#2363C6")
    form_frame.create_window(350, 35, window=title_text)
    ...
    college_search_bar = Entry(search_canvas, textvariable=colleges_var, bg="white", font=('Albert Sans', 12, 'normal'), fg="gray", borderwidth=0, highlightthickness=0)
    college_search_bar.place(x=22, y=65, width=284, height=25)
    ...
    cancel_butts = Button(college_search_bar,image=cancel,bg="white",activebackground= "white", bd=0,cursor="hand2")
    ...
    form_frame.create_window(350, 225, window=tree_frame, width=700, height=270)
    ...
    college_tree = ttk.Treeview(tree_frame, columns=columns, show="headings", yscrollcommand=scroll_y.set, selectmode="browse",height=7)
