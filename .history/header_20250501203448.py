...
def header(root):
    ...
    search_frame = Frame(top, bg="violet")
    ...
    search_canvas.grid(row=0, column=1, sticky="nsew", pady=5,padx=(0, 60))


    create_rounded_rectangle(search_canvas, 2, 2, 550, 30, radius=15, fill='#F1F3F8', outline="#E4EBF5")
    ...
    # Store a reference to the window so it can be resized later
    search_bar_window = search_canvas.create_window(10, 16, anchor="w", window=search_bar, width=530, height=25)
    ...
    def resize_search_bar(event):
        ...
        search_canvas.config(width=width)
        search_canvas.delete("rounded_bg")

        # Use the full canvas width (starting at x=0), subtracting only 1 for borders
        create_rounded_rectangle(search_canvas, 0, 0, width, 30, radius=15,
                                fill='red', outline="#E4EBF5", tag="rounded_bg")

        # Adjust the Entry widget position and width
        search_canvas.coords(search_bar_window, 10, 16)
        search_canvas.itemconfig(search_bar_window, width=width - 20)


    # Create search bar inside canvas and keep a reference
    search_bar_window = search_canvas.create_window(10, 16, anchor="w", window=search_bar, width=350, height=25)

    # Bind resize handler
    search_frame.bind("<Configure>", resize_search_bar)
