...
from misc import setup_table_deselection
...
def body(root):
    ...
    setup_table_deselection(student_table, table_frame)

    # Improved click-to-deselect functionality
    def on_click(event):
        item = student_table.identify_row(event.y)
        if item:
            current_selection = student_table.selection()
            if item in current_selection:
                # Clicked on selected item - deselect it
                student_table.selection_remove(item)
                global selected_student_id
                selected_student_id = None
            else:
                # Normal selection - clear previous and select new
    ...
    # [Rest of your existing pagination and other code remains unchanged...]
