...
def body(root):
    ...
    # Register with search module to receive updates
...
def display_students(page=0):
    ...
    # Get search text from the search module
    ...
    # Update search label to indicate what field is being searched
    search_indicator = ""
    if current_search:
        search_criteria = search.parse_search_query(current_search)
        if "general" in search_criteria and search_criteria["general"]:
            search_indicator = " (filtered)"
        else:
            # Show which fields are being searched
            search_fields = list(search_criteria.keys())
            search_indicator = f" (filtered by {', '.join(search_fields)})"
    
    total_students_label.config(text=f"Total Students: {filtered_records}{search_indicator}")
...
def on_search_changed(text):
    """
    Callback function that gets called when search text changes
    ...
    display_students(current_page)  # Refresh the table with the search filter