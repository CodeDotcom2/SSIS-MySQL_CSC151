    ...
    def submit_form():
        ...
        if edit_mode:
            ...
            if success:
                ...
                # Reset college dropdown completely
                college_var.set("")  # Clear the variable first
                college_dropdown.set("Select College")  # Set the display text
                
                # Force style reset with a delay to ensure proper rendering
                def reset_dropdown_style():
                    college_dropdown.configure(style="Gray.TCombobox")
                    college_dropdown.update()
                
                college_dropdown.after(50, reset_dropdown_style)  # Delay ensures proper reset
                
                # Update the form
                college_dropdown.update_idletasks()
                form_frame3.update_idletasks()
                form_frame3.update()  # Additional update for the canvas
