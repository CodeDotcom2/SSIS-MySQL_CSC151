...
def sort_function(body_frame):
    ...
    # Create main content frame (moved slightly higher)
    sort_frame = Frame(sort_canvas, bg="white", bd=0, highlightthickness=0)
    sort_canvas.create_window(250/2, 150, window=sort_frame, width=230, height=200)  # Changed y-position from 170 to 150
    ...
    # Create button canvas positioned higher (changed y from 300 to 280)
    button_canvas = Canvas(sort_canvas, bg="white", width=230, height=50, highlightthickness=0)
    sort_canvas.create_window(250/2, 280, window=button_canvas)  # Moved up by 20 pixels
    ...
    apply_bg = create_rounded_rectangle(button_canvas, 
                                      ...
                                      radius=20, fill="#3A7FF6", outline="#3A7FF6")
    
    cancel_bg = create_rounded_rectangle(button_canvas, 
                                       start_x + button_width + gap, 5, 
                                       start_x + (button_width * 2) + gap, 45, 
                                       radius=20, fill="#f0f0f0", outline="#f0f0f0")

    # Add centered button text
    button_canvas.create_text(start_x + button_width/2, 25, 
                            text="Apply", font=("Albert Sans", 12), fill="white")
    button_canvas.create_text(start_x + button_width + gap + button_width/2, 25, 
                            text="Cancel", font=("Albert Sans", 12), fill="black")
