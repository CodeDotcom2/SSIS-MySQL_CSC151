...
def body(root):
    ...
    # Create a container frame for the table and scrollbar
    table_container = Frame(body_frame, bg="white")
    table_container.grid(row=0, column=0, sticky="nsew", padx=20, pady=(50, 0))  # Top padding 50, bottom 0
    ...
    # Bind the configure event to check for resizing
    table_container.bind("<Configure>", lambda e: handle_resize())
...
def handle_resize():
    ...
    # Only recalculate if height changed significantly (more than 5 pixels)
    if abs(current_height - last_table_height) > 5:
        last_table_height = current_height
        new_rows = calculate_rows_per_page()
        if new_rows != items_per_page:
...
def initialize_table():
    ...
    style.configure(style_name,
                  background="white",
                  foreground="black",
                  rowheight=30,  # Set row height to 30px
...
def setup_pagination():
    ...
    # Create a container frame for pagination with 30px top margin
    pagination_container = Frame(body_frame, bg="white", height=30)
    pagination_container.grid(row=1, column=0, sticky="ew", padx=20, pady=(30, 20))  # 30px top padding
...
def calculate_rows_per_page():
    ...
    # Calculate based on actual row height (30px) and available space
    row_height = 30  # Matches the style rowheight
