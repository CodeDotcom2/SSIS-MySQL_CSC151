...
def body(root):
    ...
    body_frame.bind("<ButtonClicked-1>",remove_focus)
...
def initialize_display():
    ...
    try:
        from sorting import get_current_sort
        sort_field, sort_direction = get_current_sort()
        # Ensure name sorting uses last_name
        if sort_field == "name":
            sort_field = "last_name"
    except:
        sort_field, sort_direction = "id_number", "ascending"
    
    display_students(0, sort_field=sort_field, sort_direction=sort_direction)
...
def handle_resize(event=None, initial=False):
    ...
    if initial or abs(current_height - last_table_height) > 5:
        ...
        if new_rows != items_per_page or initial:
            ...
            # Get current sort settings when resizing
            try:
                from sorting import get_current_sort
                sort_field, sort_direction = get_current_sort()
            except:
                sort_field, sort_direction = "id_number", "ascending"
            display_students(current_page, sort_field=sort_field, sort_direction=sort_direction)
...
def initialize_table():
    ...
    columns = ("ID", "ID No.", "Name", "Gender", "Yr. Level", "Program")
...
def calculate_rows_per_page():
    ...
    row_height = 45  # Total row height including padding
...
def display_students(page=0, sort_field="id_number", sort_direction="ascending"):   
    ...
    try:
        from sorting import get_current_sort
        current_sort_field, current_sort_direction = get_current_sort()
        # Use these if they exist, otherwise use passed parameters
...
def change_page(direction):
    ...
    if 0 <= new_page < total_pages:
        ...
        # Get current sort settings from sorting module
        try:
            from sorting import get_current_sort
            sort_field, sort_direction = get_current_sort()
            # Ensure name sorting uses last_name
...
def refresh_students(sort_field="id_number", sort_direction="ascending"):
    global current_page
    display_students(current_page, sort_field=sort_field, sort_direction=sort_direction)
...
def on_search_changed(text):
    global current_page
    current_page = 0  
    # Get current sort settings when searching
    try:
        from sorting import get_current_sort
        sort_field, sort_direction = get_current_sort()
    except:
