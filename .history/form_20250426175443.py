...
def create_form(root, body_frame=None,mode="add", student_data=None):
    ...
    if mode == "edit" and student_data:
        ...
        text_elements['id_no'].insert(0, student_data['id_no'])
        ...
        text_elements['year_dropdown'].set(student_data['year_level'])
        text_elements['college_dropdown'].set(student_data['college_abbreviation'])  # or college_name if that's what you're using
        text_elements['program_dropdown'].set(student_data['program_name'])
...
def college_form(root,body_frame=None):
    ...
    college_frame = Canvas(body_frame, bg="white", width=350, height=480, bd=0, highlightthickness=0)
    form = create_rounded_rectangle(form_frame, -300, 0, 350, 480, radius=130, fill='lightgray')
    
    # Title
    title_label = Label(root, text="College Form", font=("Arial", 25, "bold"), bg="lightgray", fg="#2363C6")
    college_frame.create_window(160, 30, window=title_label)
    
    # College Name label
    name_label = Label(root, text="College Name ", bg="lightgray", font=("Arial", 15, "bold"))
    college_frame.create_window(112, 85, window=name_label)
