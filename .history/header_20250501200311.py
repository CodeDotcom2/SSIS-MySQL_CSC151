...
def header(root):
    ...
    search_field_combo = ttk.Combobox(search_frame, 
                                     ...
                                     state="readonly",
                                     font=('Albert Sans', 10))
    search_field_combo.current(0)  # Set to "All Fields"
    search_field_combo.grid(row=0, column=0, padx=(0, 10), pady=5)
    ...
    # Create a canvas to hold the search entry with rounded corners
    search_canvas = Canvas(search_frame, bg="white", height=30, bd=0, highlightthickness=0)
    search_canvas.grid(row=0, column=1, sticky="nsew", pady=5)

    # Create the rounded rectangle on the canvas
    radius = 15
    create_rounded_rectangle(search_canvas, 2, 2, 300, 30, radius=radius, fill='#F1F3F8', outline="#E4EBF5")
...
def on_field_change(event=None):
    # Get the field key based on the selected display value
    ...
    # Update the search field and trigger a search
