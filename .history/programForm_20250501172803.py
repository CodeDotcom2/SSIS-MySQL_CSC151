...
def add_program_form(root, body_frame=None, edit_mode=False, program_id=None, program_name=None, program_code=None, college_id=None):
    ...
    def submit_form():
        ...
        save_canvas.itemconfig(save_button, state="disabled")
        close_canvas.itemconfig(close, state="disabled")
        ...
        try:
            ...
            # Validation
            if not name or name == "Ex: Bachelor of Science in Information Technology":
                messagebox.showerror("Error", "Please enter a valid program name")
                return
                
            if not code or code == "Ex: BSIT":
            ...
            if not selected_college_id:
                messagebox.showerror("Error", "Please select a valid college")
                return
            ...
            if success:
                ...
                if edit_mode:
                    go_back(None)
                    refresh_students()  # Return to view form
                else:
                    # Clear form for new entry
                    add_name.delete(0, END)
                    add_name.insert(0, "Ex: Bachelor of Science in Information Technology")
                    add_name.config(fg="gray")
                    program_code_entry.delete(0, END)
                    program_code_entry.insert(0, "Ex: BSIT")
                    program_code_entry.config(fg="gray")
                    college_dropdown.set("Select College")
            else:
            ...
            # Re-enable buttons
            save_canvas.itemconfig(save_button, state="normal")
            close_canvas.itemconfig(close, state="normal")
...
def view_programs_form(root, body_frame=None):
    ...
    def load_programs():
        # Show loading state
        program_tree.delete(*program_tree.get_children())
        loading_item = program_tree.insert("", END, values=("Loading...", "", "", ""))
        
        try:
            programs = get_all_programs()
            program_tree.delete(loading_item)
