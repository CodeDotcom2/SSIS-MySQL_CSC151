...
def sort_function(body_frame):
    ...
    if sort_canvas and sort_canvas.winfo_exists():
        if is_visible:
            ...
            sort_canvas.grid(row=0, column=0, sticky="ne", padx=(0, 20), pady=(0, 20))
            is_visible = True
            if sort_button and sort_button_canvas:
                sort_button_canvas.itemconfig(sort_button, fill="#A5CAEC")
            return

    sort_canvas = Canvas(body_frame, bg="blue", width=250, height=250, highlightthickness=0, borderwidth=0)
    sort_canvas.grid(row=0, column=0, sticky="ne", padx=(0, 20), pady=(0, 20))
    is_visible = True
    ...
    sort_form = create_rounded_rectangle(sort_canvas, 0, 0, 250, 250, radius=20, fill="red")
    text = Label(sort_canvas, text="Sort Students By",font=("Albert Sans", 15, "bold"))
    text.grid(row=0, column=0, sticky=W, pady=(10, 0), padx=(20, 0))
    ...
    # Direction frame
    label_frame = Frame(sort_frame, bg="white")
    label_frame.grid(row=len(sort_options), column=0, sticky=W, pady=(5,0), padx=20)
    
    direction_label = Label(label_frame, text="Direction:", font=("Albert Sans", 14), 
                           bg="white", fg="#333333")
    direction_label.pack(side=LEFT, padx=(0, 10))
    
    direction_frame = Frame(sort_frame, bg="white")
    direction_frame.grid(row=len(sort_options)+1, column=0, sticky=E, pady=0, padx=20)
    ...
    button_frame = Canvas(sort_frame, bg="blue", width=250, height=50, highlightthickness=0, borderwidth=0)
    button_frame.grid(row=4, column=0, sticky=EW)

    apply_button = create_rounded_rectangle(button_frame, 5, 5, 122, 40, radius=20, fill="red", outline="white")

    close_button = create_rounded_rectangle(button_frame, 128, 5, 245, 40, radius=20, fill="violet", outline="white")    
